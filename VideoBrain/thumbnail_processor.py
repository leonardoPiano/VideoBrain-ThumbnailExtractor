...
def extractFramesFromSeries(nframes,serie,video, workdir = './'):
            ...
            for file in files:
                try:
                    ...
                    try:
                        ...
                        print('Try to make a prediction.')
...
def evaluateBlur(video_url,v=''):
    ...
    work_dir = os.getcwd()+'/outputs2/workdirs'
...
def extractFrames(video_url,frame_series, outputFolder, v=''):
    ...
    try:
        while True:
            ...
            if (frame_num in frame_series) == True: 
                blur_prediction = estimate_blur(frame)[0]
                cv2.imwrite(outputFolder+'/{0}/localMaxFrame_{1}_{2}.jpg'.format(v_id,frame_num,blur_prediction), frame)
            end_prediction_time = time.time()
            delay = end_prediction_time - start_prediction_time
            prediction_time = 'Blur estimation time: {0} ms blur: {1} , frame {2}'.format(delay,blur_prediction,frame_num)
            #print('prediction time: '+prediction_time)
...
def processing(youtube, yolo_v3, outputFolder, n_max_frame=5):
    ...
    workdir = extractFrames(yt,frame_series, outputFolder)
...
def execute(sourcefile, outputFolder, indexes = None):
    # class YOLO defines the default value, so suppress any default here
    # indexes is a tuple of indexes for defining the range of source videos to be analized
...
#            processing(youtube,yolo,outputFolder,5)
